...
print_summary() {
  ...
  echo ""
  echo -e "${BOLD}Cloud IDE:${NC}"
  echo -e "  URL:       ${CYAN}http://${IP}:${PORT}${NC}"
  echo -e "  Login:     ${CYAN}admin / sri@123${NC}"
  echo -e "  Dashboard: ${CYAN}http://${IP}:3000${NC}"
  echo -e "  API:       ${CYAN}http://${IP}:8100${NC}"
  echo ""
  echo -e "${BOLD}Management:${NC}"
  echo -e "  Logs:      docker compose logs -f"
  echo -e "  Stop:      docker compose down"
  echo -e "  Restart:   docker compose restart"
  echo -e "  Update:    cd ${INSTALL_DIR} && git pull && docker compose up -d"
  echo ""
}
