...
export default function NetworkPoliciesPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<{ items: NetworkPolicy[] }>({
    queryKey: ['networkpolicies', ns],
    queryFn: () => api.networkpolicies(ns),
    enabled: !!ns,
  ...
  if (isLoading) return (<div className="p-6"><LoadingState resource="Network Policies" /></div>);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-white">Network Policies</h1>
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={e => {
              if (nsParam) {
                navigate(`/namespaces/${e.target.value}/networkpolicies`);
              } else {
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Filter network policies..."
          value={filter}
      ...
      {isLoading ? (
        ...
        <div className="k-card p-0 overflow-hidden">
          <table className="k-table">
            <thead><tr><th className="pl-4">Name</th><th>Namespace</th><th>Pod Selector</th><th>Age</th></tr></thead>
            <tbody>
              {items.map(d => (
                <tr key={d.name}>
                  <td className="pl-4 font-mono text-xs text-indigo-400">{d.name}</td>
                  <td className="text-xs text-slate-500">{d.namespace}</td>
                  <td className="text-xs font-mono text-slate-400 max-w-xs truncate">{d.podSelector || '-'}</td>
                  <td className="text-xs text-slate-500">{d.created ? formatAge(d.created) : '-'}</td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan={4} className="text-center text-slate-500 py-8">{ns ? 'No network policies found' : 'Select a namespace'}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
