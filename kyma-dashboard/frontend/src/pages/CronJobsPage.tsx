...
import { Timer, Search, RefreshCw } from 'lucide-react';
...
export default function CronJobsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<{ items: CronJob[] }>({
    queryKey: ['cronjobs', ns],
    queryFn: () => api.cronjobs(ns),
    enabled: !!ns,
  ...
  if (isLoading) return (<div className="p-6"><LoadingState resource="CronJobs" /></div>);
  if (isError) return (<div className="p-6"><ErrorState title="Failed to load cronjobs" error={error} onRetry={() => refetch()} /></div>);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={e => {
              if (nsParam) {
                navigate(`/namespaces/${e.target.value}/cronjobs`);
              } else {
      ...
      {isLoading ? (
        ...
        <div className="k-card p-0 overflow-hidden">
          <table className="k-table">
            <thead><tr><th className="pl-4">Name</th><th>Namespace</th><th>Schedule</th><th>Last Schedule</th><th>Active</th><th>Age</th></tr></thead>
            <tbody>
              {items.map(d => (
                <tr key={d.name}>
                  ...
                  <td className="text-xs font-mono text-slate-300">{d.active}</td>
                  <td className="text-xs text-slate-500">{d.created ? formatAge(d.created) : '-'}</td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan={6} className="text-center text-slate-500 py-8">{ns ? 'No cronjobs found' : 'Select a namespace'}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
