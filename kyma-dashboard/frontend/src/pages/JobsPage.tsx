...
import { Workflow, Search, RefreshCw, FileText } from 'lucide-react';
...
export default function JobsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<{ items: Job[] }>({
    queryKey: ['jobs', ns],
    queryFn: () => api.jobs(ns),
    enabled: !!ns,
  ...
  if (isLoading) return (<div className="p-6"><LoadingState resource="Jobs" /></div>);
  if (isError) return (<div className="p-6"><ErrorState title="Failed to load jobs" error={error} onRetry={() => refetch()} /></div>);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={e => {
              if (nsParam) {
                navigate(`/namespaces/${e.target.value}/jobs`);
              } else {
      ...
      {isLoading ? (
        ...
        <div className="k-card p-0 overflow-hidden">
          <table className="k-table">
            <thead><tr><th className="pl-4">Name</th><th>Namespace</th><th>Completions</th><th>Status</th><th>Age</th></tr></thead>
            <tbody>
              {items.map(d => (
                <tr key={d.name}>
                  ...
                  <td className="text-xs font-mono text-slate-300">{d.completions}</td>
              ...
              {items.length === 0 && <tr><td colSpan={5} className="text-center text-slate-500 py-8">{ns ? 'No jobs found' : 'Select a namespace'}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      {yamlItem && (
        <div className="mt-3">
          <ResourceYamlPanel kind="job" name={yamlItem.name} namespace={yamlItem.namespace} onClose={() => setYamlItem(null)} />
        </div>
      )}
    </div>
  );
