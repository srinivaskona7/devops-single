...
import { Network, Search, RefreshCw, FileText } from 'lucide-react';
...
export default function IngressesPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<{ items: Ingress[] }>({
    queryKey: ['ingresses', ns],
    queryFn: () => api.ingresses(ns),
    enabled: !!ns,
  ...
  if (isLoading) return (<div className="p-6"><LoadingState resource="Ingresses" /></div>);
  if (isError) return (<div className="p-6"><ErrorState title="Failed to load ingresses" error={error} onRetry={() => refetch()} /></div>);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={e => {
              if (nsParam) {
                navigate(`/namespaces/${e.target.value}/ingresses`);
              } else {
      ...
      {isLoading ? (
        ...
        <div className="k-card p-0 overflow-hidden">
          <table className="k-table">
            <thead><tr><th className="pl-4">Name</th><th>Namespace</th><th>Class</th><th>Hosts</th><th>Age</th><th className="w-12">Actions</th></tr></thead>
            <tbody>
              {items.map(d => (
                <tr key={d.name}>
                  <td className="pl-4 font-mono text-xs text-indigo-400 cursor-pointer hover:text-indigo-300" onClick={() => setYamlItem({name: d.name, namespace: d.namespace || ns})}>{d.name}</td>
                  <td className="text-xs text-slate-500">{d.namespace}</td>
                  <td className="text-xs text-slate-400">{d.address || '-'}</td>
                  <td className="text-xs font-mono text-slate-400">{d.hosts || '-'}</td>
              ...
              {items.length === 0 && <tr><td colSpan={6} className="text-center text-slate-500 py-8">{ns ? 'No ingresses found' : 'Select a namespace'}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      {yamlItem && <div className="mt-3"><ResourceYamlPanel kind="ingress" name={yamlItem.name} namespace={yamlItem.namespace} onClose={() => setYamlItem(null)} /></div>}
    </div>
  );
}
