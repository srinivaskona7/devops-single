...
import { GitBranch, Search, RefreshCw, FileText } from 'lucide-react';
...
export default function ReplicaSetsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<{ items: ReplicaSet[] }>({
    queryKey: ['replicasets', ns],
    queryFn: () => api.replicasets(ns),
    enabled: !!ns,
  ...
  if (isLoading) return (<div className="p-6"><LoadingState resource="ReplicaSets" /></div>);
  if (isError) return (<div className="p-6"><ErrorState title="Failed to load replicasets" error={error} onRetry={() => refetch()} /></div>);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={e => {
              if (nsParam) {
                navigate(`/namespaces/${e.target.value}/replicasets`);
              } else {
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Filter replicasets..."
          value={filter}
        ...
        <div className="k-card p-0 overflow-hidden">
          <table className="k-table">
            ...
            <tbody>
              ...
              {items.length === 0 && <tr><td colSpan={4} className="text-center text-slate-500 py-8">{ns ? 'No replicasets found' : 'Select a namespace'}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      {yamlItem && (
        <div className="mt-3">
          <ResourceYamlPanel kind="replicaset" name={yamlItem.name} namespace={yamlItem.namespace} onClose={() => setYamlItem(null)} />
        </div>
      )}
    </div>
  );
