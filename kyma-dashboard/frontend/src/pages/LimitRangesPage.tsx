...
import { Ruler, Search, RefreshCw } from 'lucide-react';
...
export default function LimitRangesPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<{ items: LimitRange[] }>({
  ...
  if (isLoading) return (<div className="p-6"><LoadingState resource="Limit Ranges" /></div>);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-white">Limit Ranges</h1>
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={e => {
              if (nsParam) {
                navigate(`/namespaces/${e.target.value}/limitranges`);
              } else {
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Filter limit ranges..."
          value={filter}
      ...
      {isLoading ? (
        ...
        <div className="k-card p-0 overflow-hidden">
          <table className="k-table">
            <thead><tr><th className="pl-4">Name</th><th>Namespace</th><th>Types</th><th>Age</th></tr></thead>
            <tbody>
              {items.map(d => (
                <tr key={d.name}>
                  <td className="pl-4 font-mono text-xs text-indigo-400">{d.name}</td>
                  <td className="text-xs text-slate-500">{d.namespace}</td>
                  <td className="text-xs text-slate-400">{(d.limits || []).map(l => l.type).join(', ') || '-'}</td>
                  <td className="text-xs text-slate-500">-</td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan={4} className="text-center text-slate-500 py-8">{ns ? 'No limit ranges found' : 'Select a namespace'}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
