...
import { paths, getActiveKubeconfig, setActiveKubeconfig, getKubeconfigPath, } from '../lib/kubectl.js';
function detectAuthType(filepath) {
    try {
        const raw = fs.readFileSync(filepath, 'utf8');
        if (/^\s+token:/m.test(raw))
            return 'token';
        if (/^\s+exec:/m.test(raw))
            return 'exec/oidc';
        if (/client-certificate/.test(raw))
            return 'certificate';
    }
    catch { }
    return 'unknown';
...
function parseKubeconfig(filepath) {
    const result = { contexts: [], current_context: null, clusters: [] };
    try {
        const content = fs.readFileSync(filepath, 'utf8');
        const ccMatch = content.match(/^current-context:\s*(\S+)/m);
        if (ccMatch)
            result.current_context = ccMatch[1];
        for (const m of content.matchAll(/^\s*- name:\s*(\S+)/gm)) {
            const val = m[1];
            if (!result.contexts.includes(val))
                result.contexts.push(val);
            if (!result.clusters.includes(val))
                result.clusters.push(val);
        }
    }
    catch { }
    return result;
}
export async function kubeconfigRoutes(app) {
    ...
    app.get('/api/kubeconfig-content', async (req, reply) => {
        const kcPath = req.query.path;
        const result = { path: kcPath, content: null, contexts: [], current_context: null, clusters: [], error: null };
        if (!kcPath) {
            result.error = 'Missing required query parameter: path';
            reply.code(400);
            return result;
        }
        ...
        if (!fs.existsSync(abs)) {
            result.error = `Path does not exist: ${abs}`;
            reply.code(404);
            return result;
        }
        if (!fs.statSync(abs).isFile()) {
            result.error = `Path is not a file: ${abs}`;
            reply.code(400);
            return result;
        }
        try {
    ...
    app.post('/api/set-kubeconfig', async (req) => {
        const kcPath = req.body.path;
        if (!kcPath)
            return { success: false, path: null, error: "Missing 'path' in request body" };
        if (!path.isAbsolute(kcPath))
            return { success: false, path: null, error: 'Path must be absolute' };
        if (!fs.existsSync(kcPath))
            return { success: false, path: null, error: `File not found: ${kcPath}` };
    ...
    app.post('/api/save-kubeconfig', async (req) => {
        const { content, name, merge, set_active } = req.body;
        const result = { success: false, path: null, auth_type: null, error: null };
        ...
        if (/^\s+token:/m.test(kcContent))
            authType = 'token';
        else if (/^\s+exec:/m.test(kcContent))
            authType = 'exec/oidc';
        else if (/client-certificate/.test(kcContent))
    ...
    app.post('/api/switch-cluster', async (req) => {
        const { kubeconfig: kubeconfigYaml, clusterName } = req.body;
        if (!kubeconfigYaml)
            return { success: false, error: 'Missing kubeconfig' };
    ...
    app.get('/api/cluster-info', async () => {
        ...
        if (!kc || !fs.existsSync(kc))
            return { name: null, apiServer: null, path: null };
        try {
