...
export function getHealthData() {
        ...
        for (const res of state.resources || []) {
            if (res.type === 'btp_subaccount_environment_instance') {
                ...
                try {
                    ...
                    if (expiry) {
                        data.expiry_msg = expiry;
                        const m = expiry.match(/(\d+)/);
                        if (m)
                            data.days_left = parseInt(m[1], 10);
                    }
                ...
                try {
                    ...
                    data.modules = (params.modules?.list || []).map((m) => ({ name: m.name || '', channel: m.channel || 'regular' }));
                }
                catch { }
            }
    ...
    if (fs.existsSync(tfvarsPath)) {
        try {
            ...
            if (configured.length)
                data.modules_configured = configured;
        }
        catch { }
    }
    // Kubeconfig presence
    if (fs.existsSync(paths.TOKEN_KUBECONFIG)) {
        data.kubeconfig_type = 'token';
        data.kubeconfig_path = paths.TOKEN_KUBECONFIG;
    }
    else if (fs.existsSync(paths.OIDC_KUBECONFIG)) {
        data.kubeconfig_type = 'oidc';
        data.kubeconfig_path = paths.OIDC_KUBECONFIG;
    }
    else if (fs.existsSync(paths.OIDC_KUBECONFIG2)) {
        data.kubeconfig_type = 'oidc';
        data.kubeconfig_path = paths.OIDC_KUBECONFIG2;
    }
    // Cluster age
    if (data.created_date) {
        try {
            data.age_days = Math.floor((Date.now() - new Date(data.created_date).getTime()) / 86_400_000);
        }
        catch {
            data.age_days = null;
        }
    ...
    if (data.days_left != null) {
        try {
            ...
            data.expiry_date = data.expiry_datetime = data.expiry_iso = null;
        }
    }
    else {
        data.expiry_date = data.expiry_datetime = data.expiry_iso = null;
