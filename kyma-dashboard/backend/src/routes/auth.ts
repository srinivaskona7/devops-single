...
const _auth: Record<string, any> = { authenticated: false, username: null, token: null };
...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Querystring: { token?: string } }>('/auth/callback', async (req, reply) => {
    _auth.authenticated = true;
    _auth.username = _readBtpUsername();
    if (req.query.token) _auth.token = req.query.token;
    reply.redirect(302, '/');
    return;
  });

  // POST /api/btp-login-credentials — BTP CLI login with user/pass
  app.post<{ Body: { user: string; pass: string } }>('/api/btp-login-credentials', async (req) => {
    const { user, pass } = req.body;
    if (!user || !pass) return { success: false, error: 'Missing user/password' };
  ...
  app.get('/api/btp-login', async () => {
    try {
      ...
      if (subdomain) ssoCmd.push('--subdomain', subdomain);
      ...
      if (output.toLowerCase().includes('already logged in')) return { authenticated: true, message: 'Already logged in.' };
      return {
...
function _readBtpUsername(): string {
...
function _readSubdomain(): string | null {
  const tfvarsPath = path.join(paths.PROJECT_ROOT, 'config', 'terraform.tfvars');
  if (!fs.existsSync(tfvarsPath)) return null;
  try {
    const content = fs.readFileSync(tfvarsPath, 'utf8');
