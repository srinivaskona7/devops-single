...
export async function workloadRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Querystring: { namespace?: string } }>('/api/pods', async (req) => {
    ...
    for (const item of (r.data as any).items || []) {
      ...
      const containers = (spec.containers || []).map((c: any) => ({ name: c.name || '', image: c.image || '' }));
      const csList = status.containerStatuses || [];
      const readyCount = csList.filter((c: any) => c.ready).length;
      const totalCount = (spec.containers || []).length;
      const restarts = csList.reduce((s: number, c: any) => s + (c.restartCount || 0), 0);

      let phase = status.phase || 'Unknown';
      for (const cs of csList) {
        const waiting = cs.state?.waiting;
        if (waiting?.reason) { phase = waiting.reason; break; }
      }
  ...
  app.get<{ Querystring: { namespace?: string } }>('/api/deployments', async (req) => {
    const ns = req.query.namespace || 'default';
  ...
  app.get<{ Querystring: { namespace?: string } }>('/api/statefulsets', async (req) => {
    const ns = req.query.namespace || 'default';
  ...
  app.get<{ Querystring: { namespace?: string } }>('/api/daemonsets', async (req) => {
    const ns = req.query.namespace || 'default';
  ...
  app.get<{ Querystring: { namespace?: string } }>('/api/replicasets', async (req) => {
    const ns = req.query.namespace || 'default';
  ...
  app.get<{ Querystring: { namespace?: string } }>('/api/jobs', async (req) => {
    const ns = req.query.namespace || 'default';
  ...
  app.get<{ Querystring: { namespace?: string } }>('/api/cronjobs', async (req) => {
    const ns = req.query.namespace || 'default';
    const allNs = ['-all-', '--all-namespaces', '_all', ''].includes(ns);
    const nsFlag = allNs ? ['--all-namespaces'] : ['-n', ns];
  ...
  app.get<{ Querystring: { namespace?: string; name?: string } }>('/api/deployment-detail', async (req) => {
    const ns = req.query.namespace || 'default';
  ...
  app.get<{ Querystring: { namespace?: string; pod?: string } }>('/api/pod-detail', async (req) => {
    const ns = req.query.namespace || 'default';
    const podName = req.query.pod || '';
    if (!podName) return { data: null, error: 'Missing pod name' };
    ...
    return {
      data: {
        ...
        podIP: status.podIP,
        podIPs: (status.podIPs || []).map((p: any) => p.ip),
        ...
        volumes: (spec.volumes || []).map((v: any) => ({
          name: v.name,
          type: Object.keys(v).find((k: string) => k !== 'name') || 'unknown',
        })),
        containers: (spec.containers || []).map((c: any) => {
          const cs = (status.containerStatuses || []).find((s: any) => s.name === c.name) || {};
          return {
            name: c.name,
            image: c.image,
            imagePullPolicy: c.imagePullPolicy,
        ...
        initContainers: (spec.initContainers || []).map((c: any) => {
          const cs = (status.initContainerStatuses || []).find((s: any) => s.name === c.name) || {};
          return {
            name: c.name,
            image: c.image,
            imagePullPolicy: c.imagePullPolicy,
  ...
  app.get<{ Querystring: { kind?: string; namespace?: string; name?: string } }>('/api/manifest', async (req) => {
    const kind = req.query.kind || 'pod';
    const ns = req.query.namespace || 'default';
    const name = req.query.name || '';
    if (!name) return { yaml: '', error: 'Missing name' };

    const r = await kubectlJsonAsync([`get`, kind, name, '-n', ns]);
    if (!r.data) return { yaml: '', error: r.error };
    ...
    for (const key of Object.keys(annotations)) {
      if (systemAnnotationPrefixes.some(p => key.startsWith(p))) delete annotations[key];
    }
    if (Object.keys(annotations).length === 0) delete meta.annotations;

    const labels = meta.labels || {};
    for (const key of ['pod-template-hash', 'controller-revision-hash', 'statefulset.kubernetes.io/pod-name']) delete labels[key];
    delete obj.status;
...
function isKubectlNoise(line: string): boolean {
  return KUBECTL_NOISE.some(r => r.test(line.trim()));
}


  app.get<{ Querystring: { namespace?: string; pod?: string; container?: string; lines?: string } }>(
    '/api/pod-logs', async (req) => {
      const ns = req.query.namespace || 'default';
      const pod = req.query.pod || '';
      const container = req.query.container || '';
      const lines = Math.min(parseInt(req.query.lines || '100', 10) || 100, 10000).toString();
      if (!pod) return { logs: '', error: 'Missing required parameter: pod' };
  ...
  app.get<{ Querystring: { namespace?: string; pod?: string; container?: string; lines?: string } }>(
    '/api/pod-logs-stream', async (req, reply) => {
      const ns = req.query.namespace || 'default';
      const pod = req.query.pod || '';
      const container = req.query.container || '';
      const lines = req.query.lines || '100';

      if (!pod) { reply.status(400).send({ error: 'Missing pod' }); return; }
      ...
      req.raw.on('close', () => { try { child.kill('SIGTERM'); } catch {} });
  ...
  app.post<{ Body: { yaml: string; namespace?: string } }>('/api/apply-manifest', async (req) => {
