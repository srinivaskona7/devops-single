...
function filterHelmStderr(stderr: string): string {
  return stderr.split('\n')
...
export async function helmRoutes(app: FastifyInstance): Promise<void> {

  // GET /api/helm-releases?namespace=<ns>
  app.get<{ Querystring: { namespace?: string } }>('/api/helm-releases', async (req) => {
    ...
    try {
      ...
      result.items = releases.map((r: any) => ({
  ...
  app.post<{ Body: { chart: string; releaseName: string; namespace: string; version?: string; values?: string | Record<string, any> } }>(
    '/api/helm-install', async (req, reply) => {
      ...
      try {
        ...
        if (values && typeof values === 'string' && values.trim()) {
          ...
          if (setArgs) args.push(...setArgs.split(' '));
        }
        ...
        if (valuesFile) { try { unlinkSync(valuesFile); } catch {} }
        reply.raw.end();
      }
    },
  );
  ...
  app.post<{ Body: { releaseName: string; namespace: string; chart?: string; version?: string; values?: string | Record<string, any> } }>(
    '/api/helm-upgrade', async (req) => {
      const { releaseName, namespace, chart, version, values } = req.body;
      if (!releaseName || !namespace) return { output: '', error: 'releaseName and namespace required' };

      const result: Record<string, any> = { output: '', error: null };
      let valuesFile: string | null = null;
      try {
  ...
  app.post<{ Body: { releaseName: string; namespace: string; revision?: number } }>(
    '/api/helm-rollback', async (req, reply) => {
  ...
  // POST /api/helm-uninstall — SSE streaming
  app.post<{ Body: { releaseName: string; namespace: string } }>(
    '/api/helm-uninstall', async (req, reply) => {
      ...
      reply.raw.write(`data: $ ${cmd}\n\n`);
      try {
        const out = execSync(cmd, {
          env: getKubectlEnv(), timeout: 60_000, stdio: ['pipe', 'pipe', 'pipe'],
        });
        const lines = out.toString().trim().split('\n');
        for (const line of lines) reply.raw.write(`data: ${line}\n\n`);
        reply.raw.write(`data: SUCCESS\n\n`);
      } catch (e: any) {
        const errMsg = filterHelmStderr(e.stderr?.toString() || e.message || 'helm uninstall failed');
        for (const line of errMsg.split('\n')) reply.raw.write(`data: ERROR: ${line}\n\n`);
        reply.raw.write(`data: FAILED\n\n`);
      } finally {
        reply.raw.end();
      }
    },
  ...
  app.get('/api/helm-repo/list', async () => {
    try {
      ...
      return { repos: repos.map((r: any) => ({ name: r.name, url: r.url })), error: null };
    } catch (e: any) {
  ...
  // POST /api/helm-repo/add — add a registry with SSE streaming
  app.post<{ Body: { name: string; url: string } }>('/api/helm-repo/add', async (req, reply) => {
    ...
    try {
      ...
      const errMsg = filterHelmStderr(e.stderr?.toString() || e.message || 'helm repo add failed');
      for (const line of errMsg.split('\n')) reply.raw.write(`data: ERROR: ${line}\n\n`);
      reply.raw.write(`data: FAILED\n\n`);
    } finally {
      reply.raw.end();
  ...
  app.post<{ Body: { name: string } }>('/api/helm-repo/remove', async (req) => {
    const { name } = req.body;
  ...
  app.get<{ Querystring: { query?: string; repo?: string } }>('/api/helm-search', async (req) => {
    const query = req.query.query || '';
    const repo = req.query.repo || '';
    if (!query && !repo) return { charts: [], error: 'query required' };
  ...
  app.get<{ Querystring: { chart?: string } }>('/api/helm-chart-versions', async (req) => {
    const chart = req.query.chart || '';
    if (!chart) return { versions: [], error: 'chart required' };
    try {
      ...
      const results = JSON.parse(out.toString()) || [];
      return { versions: results.map((r: any) => r.version).filter(Boolean), error: null };
    } catch (e: any) {
  ...
  app.get<{ Querystring: { release?: string; namespace?: string } }>('/api/helm-detail', async (req) => {
    const release = req.query.release || '';
    const namespace = req.query.namespace || 'default';
    if (!release) return { error: 'release query param required' };
    ...
    const resources: { kind: string; name: string; namespace: string; apiVersion: string }[] = [];
    if (manifest) {
    ...
    const errors: string[] = [];
    if (valuesResult.status === 'rejected') errors.push(filterHelmStderr(valuesResult.reason?.stderr || valuesResult.reason?.message));
    if (manifestResult.status === 'rejected') errors.push(filterHelmStderr(manifestResult.reason?.stderr || manifestResult.reason?.message));
    if (statusResult.status === 'rejected') errors.push(filterHelmStderr(statusResult.reason?.stderr || statusResult.reason?.message));
    if (historyResult.status === 'rejected') errors.push(filterHelmStderr(historyResult.reason?.stderr || historyResult.reason?.message));
  ...
  app.get<{ Querystring: { release?: string; namespace?: string } }>('/api/helm-history', async (req) => {
    const release = req.query.release || '';
    const namespace = req.query.namespace || 'default';
    if (!release) return { items: [], error: 'release query param required' };
